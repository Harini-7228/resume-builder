...
const ExperienceForm = ({ data, onChange }) => {
    ...
    const handleAIEnhance = async (index) => {
        ...
        if (!experience.description) {
            return toast.error("Please enter some description first")
        }
        try {
                    ...
                    {data.map((experience, index) => (
                        <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3">
                            ...
                            <div className='grid md:grid-cols-2 gap-3'>
                                <input value={experience.company || ""} onChange={(e) => updateExperience(index, "company", e.target.value)} type="text" placeholder="Company Name" className="px-3 py-2 text-sm border border-gray-300 rounded-lg" />
                                <input value={experience.position || ""} onChange={(e) => updateExperience(index, "position", e.target.value)} type="text" placeholder="Job Title" className="px-3 py-2 text-sm border border-gray-300 rounded-lg" />
                                <input value={experience.start_date || ""} onChange={(e) => updateExperience(index, "start_date", e.target.value)} type="month" className="px-3 py-2 text-sm border border-gray-300 rounded-lg" />
                                <input value={experience.end_date || ""} onChange={(e) => updateExperience(index, "end_date", e.target.value)} type="month" disabled={experience.is_current} className="px-3 py-2 text-sm border border-gray-300 rounded-lg disabled:bg-gray-100" />
                            </div>
                            ...
                            <div className="space-y-2" >
                                ...
                                <textarea
                                    ...
                                    rows={4}
                                    className='w-full text-sm px-3 py-2 border border-gray-300 rounded-lg resize-none focus:ring focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors'
